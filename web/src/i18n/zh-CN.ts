...
export const zhCN: Translations = {
  ...
  search: {
    ...
    placeholder: '粘贴公开歌单链接、QQ 号或主页链接',
    parseButton: '解析',
    parsingButton: '解析中...',
    clearInput: '清空输入框',
  ...
  errors: {
    ...
    unsupportedUrl: '目前支持 QQ 音乐、网易云音乐、酷狗音乐与汽水音乐公开歌单链接、用户主页或数字 ID。',
  ...
  stats: {
    ...
    otherPlatformsComing: '酷我与更多音乐平台筹备中...',
  ...
  privacy: {
    ...
    section4Title: '4. 统计、Clarity 与 Cookie 控制',
    section4Content: '【自建统计架构（Worker / D1）】系统采用纯匿名设计，不持久化原始 IP、不写入跟踪 Cookie、不生成跨天设备指纹，也不保存用户账号或歌单歌曲内容，仅记录无用户标识的宏观聚合计数。\n\n【Microsoft Clarity】仅在您明确允许体验分析后加载。PlaylistOut 通过 Consent V2 仅授予 analytics_Storage，ad_Storage 始终保持 denied。Clarity 可能使用分析 Cookie 或伪匿名标识符连接页面与会话，并处理由 IP 派生的粗粒度网络/地域信息及页面交互遥测。\n\n【数据边界】PlaylistOut 不会通过 Clarity 的自定义事件、标签或 identify API 主动发送歌单链接、歌单 ID、歌曲名、歌手名或授权凭据。输入框、歌单摘要、歌曲列表和授权弹窗均配置 Clarity Masking。作为第三方服务，Microsoft Clarity 仍可能依据其服务机制处理页面 URL、Referrer、基础网络信息与交互数据。您的偏好仅保存在本机 localStorage，可随时在本隐私面板中修改；拒绝后，后续页面不会加载 Clarity。',
    ...
    consentBannerTitle: '体验分析与 Cookie',
    consentBannerContent: 'PlaylistOut 自建统计不使用跟踪 Cookie。Microsoft Clarity 仅在你允许后使用分析 Cookie，用于热力图、会话回放与体验改进；广告存储始终关闭。',
    consentPreferenceTitle: 'Microsoft Clarity 偏好',
    consentStatusGranted: '当前：已允许体验分析 Cookie。',
    consentStatusDenied: '当前：不使用体验分析 Cookie。',
    consentStatusUnset: '当前：尚未选择。',
    consentAllow: '允许体验分析',
    consentDecline: '不使用分析 Cookie',
    consentLearnMore: '隐私说明',
    confirmButton: '我知道了',
  },
  footer: {
